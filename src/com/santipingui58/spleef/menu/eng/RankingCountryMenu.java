...
public class RankingCountryMenu extends MenuBuilder {

	public static HashMap<Player,Integer> page = new HashMap<Player,Integer>();
	public static HashMap<Player,String> country = new HashMap<Player,String>();
	
	public RankingCountryMenu(Player p, String country) {
		super("§c§lRanking", 6);
			...
			for (String nombre : players) {
				if (Main.ranking.getConfig().getInt("players."+nombre+".position") == i) {
			        ...
			        playerMeta.setOwner(DataManager.getName(nombre));
				        playerMeta.setDisplayName("§6§l#"+i + " §d§l"+ DataManager.getName(nombre));
				       ...
				       if (country.equalsIgnoreCase(Main.ranking.getConfig().getString("players."+nombre+".IP"))) {
				       if (columna<=3) {
				        if (k<=6) {
				        	k++;
				        } else {
	...
	public void onClick(Player p, ItemStack stack, int slot) {
		if (slot == 17) {
			...
			int i = page.get(p);
			page.put(p, i-1);
			new RankingCountryMenu(p,country.get(p)).o(p);
			}
		} else if (slot ==44) {
			int next = page.get(p)+2;
